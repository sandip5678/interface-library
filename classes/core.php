...
class ShopgateLibraryException extends Exception
{
	...
	// Initialisation / instantiation of plugin failure
	...
	protected static $errorMessages
		= array(
			...
			self::PLUGIN_API_UNKNOWN_LOGTYPE        => 'unknown log type',
	 ...
	 * <br />
	 * For compatibility reasons, if an unknown error code is passed, the value is used as message
	...
	public function __construct(
		$code, $additionalInformation = null, $appendAdditionalInformationToMessage = false, $writeLog = true
	) {
...
class ShopgateMerchantApiException extends Exception
{
	...
	public function __construct($code, $additionalInformation, ShopgateMerchantApiResponse $response)
	{
		...
		if (ShopgateLogger::getInstance()->log('SMA reports error: ' . $code . ' - ' . $additionalInformation)
			=== false
		) {
			$message .= ' (unable to log)';
		}
...
class ShopgateLogger
{
	...
	/**
	 * @param null $accessLogPath
	 * @param null $requestLogPath
	 * @param null $errorLogPath
	 * @param null $debugLogPath
	 *
...
class ShopgateBuilder
{
	...
	public function buildLibraryFor(ShopgatePlugin $plugin)
	{
		...
		$authService = new ShopgateAuthentificationService(
		...
		$fileBuffer = new ShopgateFileBuffer(
...
abstract class ShopgateObject
{
	 ...
	 */
	protected $_camelizeCache = array();
	...
	/**
	 * @var array allowed parameter for magic getter & setter
	 */
	...
	public function __call($method, $args)
	{
		switch (substr($method, 0, 3)) {
			case 'get' :
				...
				if (!empty($this->_allowedParameters) && in_array($key, $this->_allowedParameters)) {
	...
	public function getData($key = '', $index = null)
	{
		...
		if (isset($this->_data[$key])) {
			...
			if (is_array($value)) {
				...
				$arr = explode("\n", $value);
				return (isset($arr[$index]) && (!empty($arr[$index]) || strlen($arr[$index]) > 0)) ? $arr[$index]
					: null;
			}
			return $default;
		}
	...
	public function camelize($str, $capitalizeFirst = false)
	{
		...
		if (empty($this->_camelizeCache[$hash])) {
			$str = strtolower($str);
			if ($capitalizeFirst) {
				$str[0] = strtoupper($str[0]);
			}

			$this->_camelizeCache[$hash] = preg_replace_callback('/_([a-z0-9])/', array($this, 'camelizeHelper'), $str);
		}

		return $this->_camelizeCache[$hash];
	}
...
abstract class ShopgatePlugin extends ShopgateObject
{
	...
	protected function removeTagsFromString($string, $removeTags = array(), $additionalAllowedTags = array())
	{
		...
		$allowedTags = array("ADDRESS", "AREA", "A", "BASE", "BASEFONT", "BIG", "BLOCKQUOTE",
							 "BODY", "BR", "B", "CAPTION", "CENTER", "CITE", "CODE", "DD", "DFN", "DIR", "DIV", "DL",
							 "DT",
							 "EM", "FONT", "FORM", "H1", "H2", "H3", "H4", "H5", "H6", "HEAD", "HR", "HTML", "IMG",
							 "INPUT",
							 "ISINDEX", "I", "KBD", "LINK", "LI", "MAP", "MENU", "META", "OL", "OPTION", "PARAM", "PRE",
	...
	protected function getMemoryUsageString()
	{
		switch (strtoupper(trim(ShopgateLogger::getInstance()->getMemoryAnalyserLoggingSizeUnit()))) {
			case 'GB':
				return
					(memory_get_usage() / (1024 * 1024 * 1024)) . " GB (real usage " . (memory_get_usage(true) / (1024
																												  * 1024
																												  * 1024))
					. " GB)";
			case 'MB':
				return
					(memory_get_usage() / (1024 * 1024)) . " MB (real usage " . (memory_get_usage(true) / (1024 * 1024))
					. " MB)";
			case 'KB':
...
abstract class ShopgateContainer extends ShopgateObject
{
	...
	public function compare($obj, $obj2, $whitelist)
	{
		foreach ($whitelist as $acceptedField) {
			if ($obj->getData($acceptedField) != $obj2->getData($acceptedField)) {
				return false;
			}
		}
		return true;
	...
	{
		$visitor
			= new ShopgateContainerUtf8Visitor(ShopgateContainerUtf8Visitor::MODE_DECODE, $destinationEncoding, $force);
		$visitor->visitContainer($this);
		return $visitor->getObject();
	}
...
class ShopgateContainerUtf8Visitor implements ShopgateContainerVisitor
{
	...
	/**
	 * @param ShopgateOrderItemOption $o
	 */
	public function visitOrderItemOption(ShopgateOrderItemOption $o)
	{
		$properties = $o->buildProperties();
	...
	/**
	 * @param ShopgateOrderItemInput $i
	 */
	public function visitOrderItemInput(ShopgateOrderItemInput $i)
	{
		$properties = $i->buildProperties();
	...
	/**
	 * @param ShopgateOrderItemAttribute $i
	 */
	...
	/**
	 * @param ShopgateOrderCustomField $c
	 */
	public function visitOrderCustomField(ShopgateOrderCustomField $c)
	{
		$properties = $c->buildProperties();
	...
	/**
	 * @param ShopgateExternalCoupon $c
	 */
	public function visitExternalCoupon(ShopgateExternalCoupon $c)
	{
		$properties = $c->buildProperties();
	...
	/**
	 * @param ShopgateShopgateCoupon $c
	 */
	public function visitShopgateCoupon(ShopgateShopgateCoupon $c)
	{
		$properties = $c->buildProperties();
	...
	/**
	 * @param ShopgateItemOptionValue $i
	 */
	public function visitItemOptionValue(ShopgateItemOptionValue $i)
	{
		$properties = $i->buildProperties();
