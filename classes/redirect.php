...
class ShopgateMobileRedirect extends ShopgateObject implements ShopgateMobileRedirectInterface
{
	...
	public function isMobileRequest()
	{
		...
		return
			(!empty($this->redirectKeywords) ? preg_match(
				'/' . implode('|', $this->redirectKeywords) . '/i', $userAgent
			) : false)
			&& (!empty($this->skipRedirectKeywords) ? !preg_match(
				'/' . implode('|', $this->skipRedirectKeywords) . '/i', $userAgent
			) : true);
	}
	...
	public function redirect($url, $autoRedirect = true)
	{
		...
		if (!$this->isRedirectAllowed() || !$this->isMobileRequest() || !$autoRedirect
			|| (($this->redirectType == 'default') && !$this->enableDefaultRedirect)
		) {
	...
	public function getMobileHeader()
	{
		...
		// set parameters
		$this->buttonOnImageSource
			= (($this->useSecureConnection) ? ShopgateMobileRedirectInterface::SHOPGATE_STATIC_SSL
				: ShopgateMobileRedirectInterface::SHOPGATE_STATIC) . '/api/mobile_header/button_on.png';
		$this->buttonOffImageSource
			  = (($this->useSecureConnection) ? ShopgateMobileRedirectInterface::SHOPGATE_STATIC_SSL
		...
		$html = str_replace(
			'{$buttonPrepend}', (($this->buttonPrepend) ? 'true' : 'false'), $html
		);
	...
	protected function getJsHeader($mobileRedirectUrl = null)
	{
		...
		if ($redirectCode == 'default') {
			$additionalParameters
				.= '_shopgate.is_default_redirect_disabled = ' . ((!$this->enableDefaultRedirect) ? 'true'
					: 'false') . ';';
		}
	...
	protected function getMobileUrl()
	{
		...
		if (!empty($this->cname)) {
			...
			$url =  'http://' . $this->alias . $this->getShopgateUrl();
		}
		return $url;
	}
	...
	protected function updateRedirectKeywords()
	{
		...
		$updateDesired = (
			...
			&& (
				...
				|| (time() - ($skipRedirectKeywordsFromFile['timestamp'] + ($this->redirectKeywordCacheTime * 3600))
					> 0)
			)
		);
		...
		if ($updateDesired) {
			try {
				...
				$this->saveKeywordsToFile(
					 $redirectKeywords, $this->config->getRedirectKeywordCachePath(), $newTimestamp
				);
				$this->saveKeywordsToFile(
					 $skipRedirectKeywords, $this->config->getRedirectSkipKeywordCachePath(), $newTimestamp
				);
			}
		}
...
/**
 * Helper class for redirection from shop system to mobile web page.
 *
 * Provides analysation of the client's user agent, creation of redirection links for
...
interface ShopgateMobileRedirectInterface
{
	...
	/**
	 * Generates a redirect to manufacturer page, if its a mobile request and parameter autoRedirectr is set to true. Otherweise the html snippet is returned
	 *
	...
	/**
	 * Create a mobile-shop-url to the start menu
	 *
	 * @deprecated
	 */
	public function getShopUrl();
